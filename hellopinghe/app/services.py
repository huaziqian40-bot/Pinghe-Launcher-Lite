...
#: 合计上限与个数上限。**刻意与网页端、PH-Launcher 保持同一组数字（20MB / 20 个）**：
#: 三端限额不一致时，同一个文件在网页端发得出去、换到客户端却被拒（或反过来），
#: 用户只会当成 bug。改这里就要同时改另外两端的同名常量。
MAIL_ATTACH_MAX_TOTAL = 20 * 1024 * 1024    # 全部附件合计上限 20MB
MAIL_ATTACH_MAX_COUNT = 20                  # 附件个数上限
