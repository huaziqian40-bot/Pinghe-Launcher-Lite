...
from ..exceptions import SchoolHubError

MAX_ROUNDS = 8
PROPOSAL_TTL = 600  # 10 分钟
SESSIONS_DIR = Path.home() / ".schoolhub" / "agent_sessions"
...
def _system_prompt(cfg: Config, workspace: str | None) -> str:
    ...
    lines = [
        "你是 SchoolHub 内置的学习助手, 运行在学生自己的电脑上.",
        f"当前时间: {_now_str()}.",
        f"学生已选科目: {subjects}.",
...
class AgentEngine:
    ...
    def new_workspace(self, name: str) -> dict:
        ...
        base = Path.home() / "Documents" / "SchoolHub"
        p = base / name
        p.mkdir(parents=True, exist_ok=True)
        self.cfg.agent_workspace = str(p)
        self._remember_workspace(str(p))
    ...
    def _resolve(self, rel: str) -> Path:
        ...
        if root is None:
            raise SchoolHubError("未设置 workspace, 请先在 Agent 页面选择")
        p = (root / rel).resolve() if rel else root
        if root.resolve() not in p.parents and p != root.resolve():
            raise SchoolHubError(f"路径越出 workspace: {rel}")
        return p
    ...
    def confirm(self, pid: str) -> dict:
        ...
        if not item:
            raise SchoolHubError(f"提案 {pid} 不存在或已过期")
        result = item["fn"]()
    ...
    def _dispatch(self, name: str, args: dict):
        ...
        if name == "get_grades":
            if not self.cfg.send_grades_to_llm:
                raise SchoolHubError("用户未允许把成绩发给 AI(设置里可开启)")
        ...
        raise SchoolHubError(f"未知工具: {name}")
