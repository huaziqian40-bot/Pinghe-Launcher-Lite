"""SchoolHub CLI —— M1 核心验证入口.

用法示例:
    python -m schoolhub.cli probe --url https://shph.managebac.cn
    python -m schoolhub.cli login --url https://shph.managebac.cn
    python -m schoolhub.cli classes
    python -m schoolhub.cli ddl --days 14 --save
    python -m schoolhub.cli timetable --days 7
    python -m schoolhub.cli preset deepseek
"""
...
def load_client(cfg: Config) -> ManageBacClient:
    if not cfg.managebac_base_url:
        raise SchoolHubError("还没配置 ManageBac 地址: schoolhub login --url https://你的学校.managebac.cn")
...
def cmd_ddl(args: argparse.Namespace) -> int:
    ...
    if args.save:
        ...
        print(f"已写入本地数据库 {n} 条 → {SESSION_DIR / 'schoolhub.db'}")
    return 0
...
def cmd_timetable(args: argparse.Namespace) -> int:
    ...
    if not (cfg.edupage_username or args.username):
        raise SchoolHubError("先在 ~/.schoolhub/config.json 填 edupage_username,或用 --username 传入")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="schoolhub", description="本地 ManageBac + Edupage 助手")
